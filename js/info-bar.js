 ...
 * 
 * Combines: Time, Weather, Space Weather Indicators, Settings, Gunny toggle
 * 
 * Events Emitted:
...
(function() {
  ...
  const styles = `
    ...
    /* GunnyGPT toggle button */
    #gunny-chat-toggle-btn.sw-indicator-gunny {
    ...
    #gunny-chat-toggle-btn.sw-indicator-gunny:hover {
      background: radial-gradient(circle at top left,
        rgba(255, 120, 0, 0.45),
        rgba(0, 0, 0, 0.9));
      box-shadow: 0 0 14px rgba(255, 120, 0, 0.7);
    }

    #gunny-chat-toggle-btn .sw-indicator-label {
  ...
  function render() {
    ...
    // Space Weather Section (including Gunny toggle)
  ...
  function renderSpaceWeatherSection() {
    ...
    // GunnyGPT Comm Planner button (always visible)
    const gunnyBtn = createGunnyButton();
    section.appendChild(gunnyBtn);
  ...
  function createGunnyButton() {
    const span = document.createElement('span');
    span.id = 'gunny-chat-toggle-btn';
    span.className = 'sw-indicator sw-indicator-gunny';
    span.title = 'GunnyGPT Comm Planner';

    const label = document.createElement('span');
    label.className = 'sw-indicator-label';
    label.textContent = '🔥 GunnyGPT';
    ...
    span.addEventListener('click', (e) => {
      ...
      if (window.GunnyChat && typeof window.GunnyChat.toggle === 'function') {
        ...
        console.warn('[InfoBar] GunnyChat module not ready yet.');
      }
    });
