...
(function() {
  ...
  function replaceGridButton() {
    ...
    Object.entries(GRID_LAYOUTS).forEach(([key, layoutConfig]) => {
      const option = document.createElement('div');
      option.className = 'grid-layout-option';
      option.textContent = layoutConfig.label;
      ...
      option.onclick = () => {
        ...
        mainBtn.textContent = `Grid: ${layoutConfig.label}`;
  ...
  function waitForHLSVideo(cellNum, attempts) {
    if (attempts > 40) { // ~4 seconds total
  ...
  function setAudioCell(cellNum) {
    ...
    document.querySelectorAll('.grid-cell-pro').forEach(cell => {
      const num = parseInt(cell.dataset.cell, 10);
      const audioBtn = cell.querySelector('.grid-audio-btn');
  ...
  window.addEventListener('load', () => {
    ...
    try {
      ...
      if (savedAudioCell) {
        audioCell = parseInt(savedAudioCell, 10);
