...
(function() {
  ...
  function addIndicators() {
    ...
    // Reserve some space on the right so we don't overlap time/weather pills
    const existingPaddingRight = parseInt(
      window.getComputedStyle(infoBar).paddingRight,
      10
    );
    if (!existingPaddingRight || existingPaddingRight < 110) {
      infoBar.style.paddingRight = '110px';
    }
    ...
    // Create container, pinned to bottom-right of the bar
  ...
  function startMaintenance() {
    // Every 6 seconds, make sure indicators still exist & have listeners
    setInterval(() => {
      const infoBar = document.getElementById('info-bar');
      if (!infoBar) return;
