  ...
  class GnssCard extends BaseCard {
    ...
    processGPData(gpData, constellationId) {
      ...
      return gpData.map(sat => {
        ...
        // Satellites with very low eccentricity and proper inclination are likely operational
    ...
    invertMatrix4x4(m) {
      // 4x4 matrix inversion for DOP calculation
    ...
    renderExpandedPanel(constellationId) {
      ...
      // Group by status
      const healthy = satellites.filter(s => s.operational);
      const unhealthy = satellites.filter(s => !s.operational);
